...
exports.getRandomRecommendationByEmotion = async (req, res) => {
    ...
    // Iteración 5: Necesitamos realizar una petición a la API De Google y obtener el libro relacionado con el ISBN y encontrar una propiedad en la respuesta de la API Google para obtener la imagen del libro. Luego, de alguna manera, hay adjuntar esta imagen en la respuesta al cliente
    /**
     * 
    "message": "Query executed successfully",
    "results": [
    ...
    console.log(data);
    // ITERACION 5: extraer la foto de la propiedad correcta de la variable data
...
exports.postBook = async (req, res) => {
    ...
    if (!errors.isEmpty()) {
        return res.status(400).json({
            message: `Error when trying to create a books`,
            errors: errors.array({ onlyFirstError: true })
        })
    }
